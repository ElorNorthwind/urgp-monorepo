...
function EquityBuildingCell(
  ...
  const styleCode =
    (rowData.developerShort === 'МОС1' ? 'ao' : 'fond') +
    (rowData.buildingIsDone ? '_done' : '_construction');
        ...
        <div className="flex flex-row gap-2 truncate">
          ...
          <div className="flex flex-1 flex-col items-start justify-start truncate">
            <div className="truncate">
              <span className="">{rowData?.addressShort}</span>
            </div>
            <div
              className={cn(
            ...
            >
              <span className="w-full truncate font-light">
                {'ЖК ' + rowData?.complexName || '-'}
              </span>
              <span className="w-full truncate font-thin">
                {rowData?.developerShort}
              </span>
            </div>
          </div>
        </div>
      ...
      <TooltipPortal>
        <TooltipContent side="bottom">
          ...
          <div className="flex max-w-[500px] flex-col gap-0">
            <div className="font-bold">{rowData?.addressShort || '-'}</div>
            <div className="flex items-start justify-between">
              <span>Кад.№:</span>
              <span className="text-muted-foreground ml-2 font-normal">
                {rowData?.buildingCadNum || '-'}
              </span>
            </div>
            <div className="flex items-start justify-between">
              <span>UNOM:</span>
