...
FROM (SELECT 
    ...
    LEFT JOIN (SELECT apartment_id, COUNT(*) as messages_count FROM renovation.messages WHERE is_deleted <> true AND apartment_id IS NOT NULL GROUP BY apartment_id) m ON a.id = m.apartment_id
${conditions:raw}
