WITH raw_data AS ( 
    ...
    SELECT DISTINCT ON(c.new_building_id)
            ...
            ST_ShortestLine(old.outline, new.outline) as path, 'movement' as type
    ...
    SELECT DISTINCT ON(c.new_building_id)
        c.new_building_id as id, new.outline, new.adress, 
        null as path, 'counstruction' as type
    ...
    SELECT DISTINCT ON(c.old_building_id)
        ...
        null as path, 'other_on_plot' as type
    FROM renovation.connection_building_construction c  
...
SELECT 
    ...
    ST_AsGeoJSON(ST_LineInterpolatePoint(path, 0.1))::jsonb as start,
    ST_AsGeoJSON(ST_LineInterpolatePoint(path, 0.9))::jsonb as finish,    
    type,
    ST_AsGeoJSON(ST_Extent(outline) OVER())::jsonb as bounds
FROM raw_data;