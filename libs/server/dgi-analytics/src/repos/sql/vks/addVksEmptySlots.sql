WITH full_slots AS (
    SELECT 
		...
		slot_num
    FROM 
        (SELECT *, generate_series(1, slots_per_time) as slot_num FROM vks.slot_schedule) v
), short_slots AS (
    SELECT 
        group_id,
        week_day,
    ...
    SELECT
        ...
        LPAD(t.group_id::text, 2, '0') || '-' || TO_CHAR(t.date, 'DD.MM.YYYY') || '-' || LEFT(t.slot_text, 5) || '-' || COALESCE(t.slot_num, 1)::text as booking_code,
    ...
    LEFT JOIN (
        SELECT -- DISTINCT ON (se.slot_group_id, ca.date, ca.time) 
            ca.*, 
            se.slot_group_id,
			ROW_NUMBER() OVER (PARTITION BY se.slot_group_id, ca.date, ca.time ORDER BY ca.id DESC)::int as slot_num
        FROM vks.cases ca
    ...
    ) c ON c.date = t.date AND c.time = t.slot_text AND c.slot_group_id = t.group_id AND c.slot_num = t.slot_num
    ...
    LEFT JOIN (
        ...
        ORDER BY se.slot_group_id, ca.date, ca.time, ca.created_at DESC
    ) ex ON ex.date = t.date AND ex.time = t.slot_text AND ex.slot_group_id = t.group_id AND ex.slot_num = t.slot_num
