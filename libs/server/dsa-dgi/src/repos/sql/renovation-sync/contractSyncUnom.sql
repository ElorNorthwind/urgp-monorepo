...
SELECT DISTINCT ON (ao.id, an.id)
    ...
    CASE 
        WHEN ugd_notifications[1]->>'notification_date' IS NOT NULL THEN (ugd_notifications[1]->>'notification_date')::date
        WHEN c.appids IS NOT NULL THEN c.contract_date_created 
        ELSE NULL 
    END as contract_notification_date,
    CASE 
        WHEN ugd_notifications[1]->>'notification_number' IS NOT NULL THEN ugd_notifications[1]->>'notification_number'
        WHEN c.appids IS NOT NULL THEN ARRAY_TO_STRING(c.appids, '; ') 
        ELSE NULL 
    END as contract_notification_num
FROM public.contracts c 
LEFT JOIN public.new_apart a ON a.unom = c.new_apart_unom AND a.un_kv = c.new_apart_unkv
...
WHERE ao.id IS NOT NULL AND an.id IS NOT NULL AND c.area_id IS NULL
