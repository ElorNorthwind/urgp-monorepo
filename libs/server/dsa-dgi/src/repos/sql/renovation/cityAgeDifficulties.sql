WITH apartment_totals AS (
    SELECT 
        building_id,
        COUNT(*) FILTER (WHERE (classificator->>'deviation')::varchar = ANY(ARRAY['Риск', 'Требует внимания'])) as bad        FROM renovation.apartments_old
    ...
    SELECT 
        ...
        CASE
            ...
            ELSE 'Переселение'
        END as status,
        COALESCE(b.manual_relocation_type, b.relocation_type) as type_id,
        CASE
...
WHERE b.status = 'Переселение' AND b.type_id = 1 AND b.deviation = 'Нехороший дом' AND a.classificator->>'deviation' = ANY(ARRAY['Риск', 'Требует внимания'])
