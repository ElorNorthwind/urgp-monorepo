...
INSERT INTO control.direction_types (id, name, category, fullName, tags) VALUES 
...
--     directiom_id integer REFERENCES control.direction_types(id) ON DELETE CASCADE,
-- 	author_id integer REFERENCES renovation.users(id) ON DELETE SET NULL, -- а не хотим ли мы своих юзеров с блекджеком?
-- 	PRIMARY KEY (id)
-- );
...
INSERT INTO control.operation_types (id, name, category, fullName, priority, auto_approve) VALUES 
(1, 'запрос', 'рассмотрение', 'Направлен запрос', 1, true),
(2, 'получено', 'рассмотрение', 'Получены документы', 1, true),
(3, 'письмо', 'рассмотрение', 'Направлено письмо', 1, true),
(4, 'звонок', 'рассмотрение', 'Устные разъяснения', 1, true),
(5, 'заявка', 'рассмотрение', 'Создана заявка', 1, true),
(6, 'проработка', 'рассмотрение', 'Иная операция', 1, true),

(7, 'отклонено (исп)', 'решение', 'По заявке не выявлено проблемы', 2, false),
(8, 'решено (исп)', 'решение', 'Описанная проблема решена', 2, false),
(9, 'нерешаемо (исп)', 'решение', 'Решить Проблему не удалось', 2, false);
