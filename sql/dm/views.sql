...
----------------------------------------------------------------------
    ...
    WHERE r.control_date >= '01.01.2025'::timestamp with time zone AND c.id IS NOT NULL AND c.category_group = 'EDO'
...
----------------------------------------------------------------------
    ...
    WHERE c.id IS NOT NULL AND r.control_date BETWEEN '01.01.2025'::timestamp AND date_limit.date AND c.category_group = 'EDO'
...
-- Описанное в ТЗ представление по заявкам СПД
...
----------------------------------------------------------------------
    ...
    LEFT JOIN dm.departments dep ON c.department_id = dep.id
    LEFT JOIN dm.zams z ON dep.zam_id = z.id
    WHERE d.reg_date >= '01.01.2025'::timestamp with time zone AND c.id IS NOT NULL AND c.category_group = 'SPD'
...
-- Представление до 3 последних рабочих дней
DROP VIEW IF EXISTS dm.spd_dated_view CASCADE;
CREATE OR REPLACE VIEW dm.spd_dated_view  AS
----------------------------------------------------------------------
    WITH date_limit AS (
        SELECT date
        FROM dm.calendar
    ...
    LEFT JOIN dm.departments dep ON c.department_id = dep.id
    LEFT JOIN dm.zams z ON dep.zam_id = z.id, date_limit
    WHERE c.id IS NOT NULL AND d.reg_date BETWEEN '01.01.2025'::timestamp AND date_limit.date AND c.category_group = 'SPD'
    ORDER BY d.reg_date;
----------------------------------------------------------------------
