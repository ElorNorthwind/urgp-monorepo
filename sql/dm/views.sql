...
-- Представление ЭДО + СПД до 3 последних рабочих дней
DROP VIEW IF EXISTS dm.documents_combined_dated_view CASCADE;
CREATE OR REPLACE VIEW dm.documents_combined_dated_view  AS
----------------------------------------------------------------------
    WITH date_limit AS (
        SELECT date
        FROM dm.calendar
