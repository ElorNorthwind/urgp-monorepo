...
INSERT INTO equity.object_status_types (id, name, priority) VALUES
    (1, 'Не обращался', 1),
    (2, 'Акт подписан', 3),
    (3, 'Передано дольщику', 4),
    (4, 'Подрежит передаче городу', 5),
    (5, 'Передано городу', 6),
    (6, 'Статус не определен', 7)
    (7, 'Рассмотрение документов', 2);
...
ALTER TABLE equity.operation_types
    OWNER to renovation_user;
INSERT INTO equity.operation_types (id, name, fullname, category, priority, order) VALUES
(6, 'Запрошено заключение - подписать акт', 'Запрошено заключени о возможности подписать акт', '1. заключения', 4, 12),
(11, 'Запрошено заключение - выдать ключи', 'Запрошено заключени о возможности выдачи ключей', '1. заключения', 4, 13),
(14, 'Заключение УПОЖС дано', 'Ответ УПОЖС (Спесивцева С.В.)', '1. заключения', 5, 14),
(15, 'Заключение УОРК дано', 'Ответ УОРК (Стифеева А.С.)', '1. заключения', 5, 15),
(7, 'Заключение УРЖП дано', 'Ответ УРЖП (Лукьянов М.Г.)', '1. заключения', 5, 16),

(5, 'Запись на прием - подача документов', 'Дольщик записался на подачу документов', '2. прием', 3, 9),
(12, 'Запись на прием - консультация', 'Заявитель записался на консультацию', '2. прием', 2, 10),
(13, 'Консультация оказана', 'Заявителю дана консультация', '2. прием', 3, 11),

(1, 'Акт подписан', 'Подписан акт приема-передачи помещения', '3. выдача', 10, 17),
(16, 'Выданы ключи', 'Дольщику выданы ключи от помещения', '3. выдача', 11, 18),

(4, 'Дефекты выявлены', 'При осмотре выявлены дефекты', '4. дефекты', 1, 6),
(2, 'Дефектная ведомость открыта', 'Открыта дефектная ведомость', '4. дефекты', 1, 7),
(3, 'Дефектная ведомость закрыта', 'Закрыта дефектная ведомость', '4. дефекты', 2, 8), 

(17, 'Примечание', 'Информация по помещению', '5. информация', 0, 1),
(8, 'Выявлены признаки двойной продажи', 'По квартире выявлены конкурирующие требования', '5. информация', 0, 2),
(18, 'Двойная продажа не подтверждена', 'Сделан вывод об отсутствии двойной продажи', '5. информация', 1, 3),
(9, 'Выявлены проблемы идентификации', 'Имеются сомнения о соответствии требования данному помещению', '5. информация', 0, 4),
(10, 'Проблемы идентификации сняты', 'Сомнения в соответствии требования и помещения сняты', '5. информация', 1, 5);
