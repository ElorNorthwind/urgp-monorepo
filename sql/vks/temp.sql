WITH full_slots AS (
    ...
    WHERE c.is_workday AND s.group_id IS NOT NULL AND c.date BETWEEN '01.01.2025'::date AND '29.01.2026'::date
), empty_slots AS (
    SELECT
    ...
    LEFT JOIN (
        ...
        WHERE ca.status NOT IN ('обслужен', 'не явился по вызову', 'пустой слот')
    ...
    LEFT JOIN (
        ...
        FROM vks.cases ca
        LEFT JOIN vks.services se ON ca.service_id = se.id
        WHERE ca.status IN ('обслужен', 'не явился по вызову', 'пустой слот')
        ORDER BY se.slot_group_id, ca.date, ca.time, ca.created_at DESC
